...
function makeResponseId(response) {
  return [response.year, safeId(response.meetingTitle), safeId(response.team), safeId(response.name)].join("_");
}
...
async function seedDefaultDataIfEmpty() {
  ...
  await setDoc(doc(db, "settings", "main"), { meetingTitle: "연합기도회", updatedAt: new Date().toISOString() }, { merge: true });
}
...
function runSelfTests() {
  ...
  console.assert(s.불참 === 1, "불참 수 계산 오류");
  console.assert(getMainMeetingTitle("NCMN 중보기도팀 모임") === "중보기도팀 모임", "모임명 표시 오류");
  console.assert(makeResponseId({ year: 2026, meetingTitle: "연합기도회", team: "A팀", name: "가" }).includes("2026"), "응답 ID 생성 오류");
}
runSelfTests();
...
function App() {
  ...
  async function handleUpdateMeetingTitle() {
    ...
    if (!title) return alert("모임 제목을 입력해주세요.");
    await setDoc(doc(db, "settings", "main"), { meetingTitle: title, updatedAt: new Date().toISOString() }, { merge: true });
  }
    ...
    <main className="page">
      <div className="container">
        <motion.section initial={{ opacity: 0, y: 16 }} animate={{ opacity: 1, y: 0 }} transition={{ duration: 0.5 }} className="hero">
          ...
          <div className="meetingTitle">{getMainMeetingTitle(meetingTitle)}</div>
            ...
            {!isAdmin ? (
              ...
              <div className="stack">
                ...
                <div className="summaryGrid">
                  <div className="summary"><p>전체 응답</p><strong>{summary.total}</strong></div>
                ...
                <div>
                  <h3>응답 목록</h3>
                  <div className="list">
                    {responseList.length ? responseList.map((item, i) => (
                ...
                <div className="panel">
                  ...
                  <div className="manage">
                    ...
                    <div className="options">
                      {meetingOptions.map((option) => <button key={option} type="button" onClick={() => setMeetingTitleDraft(option)} className={meetingTitleDraft === option ? "option active" : "option"}>{option}</button>)}
                    </div>
                    <div className="inputRow"><input value={meetingTitleDraft} onChange={(e) => setMeetingTitleDraft(e.target.value)} /><Button type="button" onClick={handleUpdateMeetingTitle}>반영</Button></div>
                  </div>
              ...
              <Section title="이름 선택">
                <div className="buttonGrid">
                  {availableMembers.length ? availableMembers.map((member) => {
                    const submitted = responseList.some((r) => r.team === form.team && r.name === member && r.meetingTitle === meetingTitle);
