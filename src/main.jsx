...
function makeResponseId(response) {
  return [
    response.year,
    safeId(response.meetingTitle),
    safeId(response.team),
    safeId(response.name),
  ].join("_");
}

async function seedDefaultTeamsIfEmpty() {
  if (!db) return;
  const teamsSnapshot = await getDocs(collection(db, "teams"));
  if (!teamsSnapshot.empty) return;

  await Promise.all(
    Object.entries(INITIAL_TEAMS).map(([teamName, members]) =>
      setDoc(doc(db, "teams", safeId(teamName)), {
        name: teamName,
        members,
...
function runSelfTests() {
  const testSummary = buildSummary([
    { team: "A팀", name: "가", attendance: "참석", absentReason: "" },
    { team: "A팀", name: "나", attendance: "불참", absentReason: "일정" },
    { team: "B팀", name: "다", attendance: "참석", absentReason: "" },
  ]);

  console.assert(testSummary.total === 3, "전체 응답 수 계산 오류");
  console.assert(testSummary.참석 === 2, "참석 수 계산 오류");
  console.assert(testSummary.불참 === 1, "불참 수 계산 오류");
  console.assert(testSummary.teams.A팀.total === 2, "팀별 전체 수 계산 오류");
  console.assert(getMainMeetingTitle("연합기도회") === "연합기도회", "연합기도회 표시 오류");
  console.assert(getMainMeetingTitle("NCMN 중보기도팀 모임") === "중보기도팀 모임", "중보기도팀 모임 표시 오류");
  ...
  console.assert(responseId.includes("2026"), "응답 ID 생성 오류");
}
...
export default function TeamMeetingAttendancePage() {
  ...
  useEffect(() => {
    if (!hasFirebaseConfig || !db) {
      setLoadMessage("Firebase 설정이 아직 연결되지 않았습니다. .env 값을 입력 후 다시 배포해주세요.");
      return;
    }

    seedDefaultTeamsIfEmpty().catch((error) => {
      console.error(error);
      setLoadMessage("초기 팀 목록을 저장하는 중 문제가 발생했습니다.");
    });
    ...
    const unsubTeams = onSnapshot(collection(db, "teams"), (snapshot) => {
      ...
      snapshot.forEach((teamDoc) => {
        ...
        if (data.name) nextTeams[data.name] = data.members || [];
      });
      if (Object.keys(nextTeams).length) setTeams(nextTeams);
    });

    const unsubResponses = onSnapshot(collection(db, "responses"), (snapshot) => {
      const nextResponses = [];
      snapshot.forEach((responseDoc) => {
        nextResponses.push({ id: responseDoc.id, ...responseDoc.data() });
      });
      nextResponses.sort((a, b) => new Date(b.createdAt || 0) - new Date(a.createdAt || 0));
      setResponseList(nextResponses);
    });
    ...
    const unsubSettings = onSnapshot(doc(db, "settings", "main"), (snapshot) => {
      ...
      if (data.meetingTitle) {
        ...
        setMeetingTitleDraft(data.meetingTitle);
      }
    });
  ...
  const handleSubmit = async (event) => {
    ...
    await setDoc(doc(db, "responses", makeResponseId(nextResponse)), nextResponse);
  ...
  const handleAddTeam = async () => {
    ...
    await setDoc(doc(db, "teams", safeId(teamName)), {
      name: teamName,
      members: [],
      updatedAt: new Date().toISOString(),
    });
  ...
  const handleDeleteTeam = async () => {
    ...
    const deleteTargets = responseList.filter((item) => item.team === deleteTeamName);
    await Promise.all(deleteTargets.map((item) => deleteDoc(doc(db, "responses", item.id))));
  ...
  const handleDeleteMember = async () => {
    ...
    await setDoc(doc(db, "teams", safeId(deleteMemberTeam)), {
      ...
      members: (teams[deleteMemberTeam] || []).filter((member) => member !== deleteMemberName),
      updatedAt: new Date().toISOString(),
    });

    const deleteTargets = responseList.filter(
      (item) => item.team === deleteMemberTeam && item.name === deleteMemberName
    );
    await Promise.all(deleteTargets.map((item) => deleteDoc(doc(db, "responses", item.id))));
              ...
              <div>
                ...
                <div className="button-grid names">
                  {availableMembers.length ? availableMembers.map((member) => {
                    const submitted = responseList.some(
                      (item) => item.team === form.team && item.name === member && item.meetingTitle === meetingTitle
                    );
                    return (
                      <button
                        key={member}
